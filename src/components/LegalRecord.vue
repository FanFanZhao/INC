      ...
      <li v-for="(item,index) in list" :key="index" class="eee">
        <div class="flex li-t">
          ...
          <div class="status">
            ...
            <router-link class="redColor"  :to="{path:'/legalPayDetail',query:{id:item.id}}" v-else-if="item.is_sure == 1">已完成 ></router-link>
            <router-link class="ceilColor" :to="{path:'/legalPayDetail',query:{id:item.id}}" v-else-if="item.is_sure == 2">已取消 ></router-link>
            <router-link class="base"  :to="{path:'/legalPayDetail',query:{id:item.id}}" v-else>已付款 ></router-link>
